...
// アトラステクスチャ（atlas_texture配列の内容はatlas.inlから取得）
// 注意: ここでは空のデータを使用し、後で特定の領域を白色に設定します
...
static mu_Rect atlas[] = {
    ...
    // ATLAS_FONT (MU_ICON_MAX + 1)から始まるフォントデータ
...
HRESULT dx11_init(HWND hWnd, int width, int height) {
    ...
    // ??重要: 白色パッチ領域を白色に設定（ATLAS_WHITEの位置）
    ...
    for (int y = 0; y < closeIcon.h; y++) {
        for (int x = 0; x < closeIcon.w; x++) {
            ...
            if (abs(dx) == abs(dy) || abs(dx) == abs(dy) - 1 || abs(dx) - 1 == abs(dy)) {
                atlas_texture[idx] = 255; // Xの形を白で描画
            }
        }
    }

    // ??重要: 完全に白いテクスチャを作成（全ピクセルをRGBA=(255,255,255,255)に）
    ...
    samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT; // 最近傍サンプリングでクリアなフォントに
...
void dx11_cleanup(void) {
    ...
    if (g_rasterizerState) g_rasterizerState->lpVtbl->Release(g_rasterizerState); // 追加
...
// 四角形を頂点バッファに追加する（UV座標指定なし - 基本的に使用しない）
    ...
    for (int i = 0; i < command_count; i++) {
        ...
        switch (cmd->type) {
            case MU_COMMAND_RECT:
                // 重要: 単純なdx11_push_quadではなく、atlas[ATLAS_WHITE]を使用するよう修正
            ...
            case MU_COMMAND_ICON:
                dx11_draw_icon(cmd->rect.w, cmd->rect, cmd->color); // rect.wにアイコンIDを格納
                break;
        }
    }
