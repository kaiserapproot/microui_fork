...
- DirectX11でTTFフォントテクスチャ生成の流れ
- TTFモード時はatlas.inlを一切使わずTTFアトラスのみ参照
- UI描画時のwhite_patch領域の重要性
- 日本語表示のためのShift-JIS→UTF-8変換の必要性
- ベースライン補正の値は「+8」などプラス値で調整

---

**この履歴は、TTFフォント対応における日本語表示・文字化け・UI描画不具合の原因と修正の詳細をまとめたものです。**