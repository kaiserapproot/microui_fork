...
enum {
    ...
    MU_COLOR_BASE,         /* ??UI???? */
    MU_COLOR_BASEHOVER,    /* ??UI????????? */
    MU_COLOR_BASEFOCUS,    /* ??UI??????????? */
    MU_COLOR_SCROLLBASE,   /* ???????????? */
...
/* ?????? */
enum {
    MU_KEY_SHIFT = (1 << 0),      /* Shift?? */
    MU_KEY_CTRL = (1 << 1),       /* Ctrl?? */
    MU_KEY_ALT = (1 << 2),        /* Alt?? */
    MU_KEY_BACKSPACE = (1 << 3),  /* ????????? */
    MU_KEY_RETURN = (1 << 4)      /* Enter?? */
};
...
struct mu_Context {
    ...
    void (*draw_text)(mu_Context* ctx, mu_Font font, const char* str, int len, mu_Vec2 pos, mu_Color color); /* ??????????? */
    ...
    mu_stack(mu_Id, MU_IDSTACK_SIZE) id_stack;                 /* ID???????? */
