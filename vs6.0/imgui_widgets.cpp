...
static int i, n, item_n, picker_type, tab_src_n, tab_n, section_n;
...
// int型専用のCheckboxFlags関数を実装
static bool CheckboxFlagsInt(const char* label, int* flags, int flags_value)
{
...
#if _MSC_VER <= 1200 // VC++6.0
    // 使用箇所の修正
    return CheckboxFlagsInt(label, flags, flags_value);
#else
...
#if _MSC_VER <= 1200 // VC++6.0
    // 使用箇所の修正
    return CheckboxFlagsInt(label, (int*)flags, flags_value);
#else
    return CheckboxFlagsT(label, flags, flags_value);
#endif
}
...
//	#if _MSC_VER <= 1200 // VC++6.0
//	// 使用箇所の修正
//	return CheckboxFlagsInt(label, (mS64*)flags, flags_value);
//	#else
//    return CheckboxFlagsT(label, flags, flags_value);
//	#endif
//}
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    ...
    while (width_excess > 0.0f && count_same_width < count)
    {
        ...
        for (item_n = 0; item_n < count_same_width; item_n++)
...
// 整数型用の丸め処理関数
static ImS32 RoundScalarWithFormatS32(const char* format, ImS32 v)
{
    if (format == NULL)
        return v;
    ...
    // フォーマット文字列からの精度の取得
    char fmt_backup[64];
    strncpy(fmt_backup, format, 63);
    fmt_backup[63] = 0;

    // %d または %i フォーマットのみをサポート
    // 他のフォーマットの場合は値をそのまま返す
    if (strstr(fmt_backup, "%d") || strstr(fmt_backup, "%i"))
    {
        char buf[64];
        ImFormatString(buf, 64, format, v);
        return (ImS32)atoi(buf);
    }

    return v;
}
// 浮動小数点数の丸め処理用関数
static float RoundScalarWithFormatFloat(const char* format, float v)
{
    if (format == NULL)
        return v;
    ...
    if (dot && dot[1] >= '0' && dot[1] <= '9')
    {
        ...
        {
            // 指定された精度で丸める
            float power = (float)pow(10.0f, (float)precision);
            v = (float)(floor((double)(v * power + 0.5f)) / power);
        }
    }
...
// double型の丸め処理用関数
static double RoundScalarWithFormatDouble(const char* format, double v)
{
    if (format == NULL)
        return v;
    ...
    if (dot && dot[1] >= '0' && dot[1] <= '9')
    {
        ...
        {
            // 指定された精度で丸める
            double power = pow(10.0, (double)precision);
            v = floor((v * power) + 0.5) / power;
        }
    }
...
bool DragBehaviorS32_S8(ImGuiDataType data_type, ImS32* v32, float v_speed,
    ImS32 v_min, ImS32 v_max,
    const char* format, ImGuiSliderFlags flags)
{
    ...
    // マウス入力の処理
    float adjust_delta = 0.0f;
    if (g.ActiveIdSource == ImGuiInputSource_Mouse && ImGui::IsMousePosValid() &&
        ImGui::IsMouseDragPastThreshold(0, g.IO.MouseDragThreshold * DRAG_MOUSE_THRESHOLD_FACTOR))
    {
        adjust_delta = g.IO.MouseDelta[axis];
        if (g.IO.KeyAlt)
            adjust_delta *= 1.0f / 100.0f;
    ...
    {
        adjust_delta = ImGui::GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad,
    ...
    bool is_already_past_limits_and_pushing_outward = is_clamped &&
        ((*v32 >= v_max && adjust_delta > 0.0f) || (*v32 <= v_min && adjust_delta < 0.0f));
    ...
    ImS32 v_cur = *v32;
    v_cur += (ImS32)g.DragCurrentAccum;
    g.DragCurrentAccum -= (float)((ImS32)v_cur - (ImS32)*v32);
    g.DragCurrentAccumDirty = false;
...
bool DragBehaviorU32_U8(ImGuiDataType data_type, ImU32* v32, float v_speed,
    ImU32 v_min, ImU32 v_max,
    const char* format, ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiAxis axis = (flags & ImGuiSliderFlags_Vertical) ? ImGuiAxis_Y : ImGuiAxis_X;
    ...
    ImU32 v_cur = *v32;
    v_cur += (ImU32)g.DragCurrentAccum;
    g.DragCurrentAccum -= (float)((ImS32)v_cur - (ImS32)*v32);
    g.DragCurrentAccumDirty = false;
...
bool DragBehaviorS32_S16(ImGuiDataType data_type, ImS32* v32, float v_speed,
    ImS32 v_min, ImS32 v_max,
    const char* format, ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
...
bool DragBehaviorU32_U16(ImGuiDataType data_type, ImU32* v32, float v_speed,
    ImU32 v_min, ImU32 v_max,
    const char* format, ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
...
bool DragBehaviorS32_S32(ImGuiDataType data_type, ImS32* p_v, float v_speed,
    ImS32 v_min, ImS32 v_max,
    const char* format, ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    bool is_just_activated = g.ActiveIdIsJustActivated;
    bool is_already_past_limits_and_pushing_outward = is_clamped &&
        ((*p_v >= v_max && adjust_delta > 0.0f) || (*p_v <= v_min && adjust_delta < 0.0f));
    ...
    ImS32 v_cur = *p_v;
    v_cur += (ImS32)g.DragCurrentAccum;
    g.DragCurrentAccum -= (float)((ImS32)v_cur - (ImS32)*p_v);
    g.DragCurrentAccumDirty = false;
...
bool DragBehaviorU32(ImGuiDataType data_type, ImU32* p_v, float v_speed,
    ImU32 v_min, ImU32 v_max,
    const char* format, ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
...
bool DragBehaviorFloat(ImGuiDataType data_type, float* p_v, float v_speed,
    ...
    const char* format, ImGuiSliderFlags flags)
{
    ...
    {
        // 対数スケールでの値の更新
        float v_old_log = (float)log(v_cur);
        float v_new_log = v_old_log + g.DragCurrentAccum;
        v_cur = (float)exp(v_new_log);
    }
    else
    {
...
bool DragBehaviorDouble(ImGuiDataType data_type, double* p_v, float v_speed,
    ...
    const char* format, ImGuiSliderFlags flags)
{
    ...
    // デフォルトの調整速度
    if (v_speed == 0.0f && is_clamped && (v_max - v_min < DBL_MAX))
        v_speed = (float)((v_max - v_min) * g.DragSpeedDefaultRatio);
    ...
    // 対数スケールの処理
    if (is_logarithmic && (v_max - v_min < DBL_MAX) && ((v_max - v_min) > 0.000001))
        adjust_delta /= (float)(v_max - v_min);
    ...
    {
        // 対数スケールでの値の更新
        double v_old_log = log(v_cur);
        double v_new_log = v_old_log + g.DragCurrentAccum;
        v_cur = exp(v_new_log);
    }
    else
    {
...
//    case ImGuiDataType_U8: { ImU32 v32 = (ImU32) * (ImU8*)p_v;  bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed, p_min ? *(const ImU8*)p_min : IM_U8_MIN, p_max ? *(const ImU8*)p_max : IM_U8_MAX, format, flags); if (r) *(ImU8*)p_v = (ImU8)v32; return r; }
    case ImGuiDataType_U8: { ImU32 v32 = (ImU32) * (ImU8*)p_v;  bool r = DragBehaviorU32_U8(ImGuiDataType_U32, &v32, v_speed, p_min ? *(const ImU8*)p_min : IM_U8_MIN, p_max ? *(const ImU8*)p_max : IM_U8_MAX, format, flags); if (r) *(ImU8*)p_v = (ImU8)v32; return r; }
//    case ImGuiDataType_S16: { ImS32 v32 = (ImS32) * (ImS16*)p_v; bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed, p_min ? *(const ImS16*)p_min : IM_S16_MIN, p_max ? *(const ImS16*)p_max : IM_S16_MAX, format, flags); if (r) *(ImS16*)p_v = (ImS16)v32; return r; }
    case ImGuiDataType_S16: { ImS32 v32 = (ImS32) * (ImS16*)p_v; bool r = DragBehaviorS32_S16(ImGuiDataType_S32, &v32, v_speed, p_min ? *(const ImS16*)p_min : IM_S16_MIN, p_max ? *(const ImS16*)p_max : IM_S16_MAX, format, flags); if (r) *(ImS16*)p_v = (ImS16)v32; return r; }
//    case ImGuiDataType_U16: { ImU32 v32 = (ImU32) * (ImU16*)p_v; bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed, p_min ? *(const ImU16*)p_min : IM_U16_MIN, p_max ? *(const ImU16*)p_max : IM_U16_MAX, format, flags); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
    case ImGuiDataType_U16: { ImU32 v32 = (ImU32) * (ImU16*)p_v; bool r = DragBehaviorU32_U16(ImGuiDataType_U32, &v32, v_speed, p_min ? *(const ImU16*)p_min : IM_U16_MIN, p_max ? *(const ImU16*)p_max : IM_U16_MAX, format, flags); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
//    case ImGuiDataType_S32:    return DragBehaviorT<ImS32, ImS32, float>(data_type, (ImS32*)p_v, v_speed, p_min ? *(const ImS32*)p_min : IM_S32_MIN, p_max ? *(const ImS32*)p_max : IM_S32_MAX, format, flags);
    case ImGuiDataType_S32:    return DragBehaviorS32_S32(data_type, (ImS32*)p_v, v_speed, p_min ? *(const ImS32*)p_min : IM_S32_MIN, p_max ? *(const ImS32*)p_max : IM_S32_MAX, format, flags);
//    case ImGuiDataType_U32:    return DragBehaviorT<ImU32, ImS32, float>(data_type, (ImU32*)p_v, v_speed, p_min ? *(const ImU32*)p_min : IM_U32_MIN, p_max ? *(const ImU32*)p_max : IM_U32_MAX, format, flags);
    case ImGuiDataType_U32:    return DragBehaviorU32(data_type, (ImU32*)p_v, v_speed, p_min ? *(const ImU32*)p_min : IM_U32_MIN, p_max ? *(const ImU32*)p_max : IM_U32_MAX, format, flags);
...
//    case ImGuiDataType_Float:  return DragBehaviorT<float, float, float >(data_type, (float*)p_v, v_speed, p_min ? *(const float*)p_min : -FLT_MAX, p_max ? *(const float*)p_max : FLT_MAX, format, flags);
    case ImGuiDataType_Float:  return DragBehaviorFloat(data_type, (float*)p_v, v_speed, p_min ? *(const float*)p_min : -FLT_MAX, p_max ? *(const float*)p_max : FLT_MAX, format, flags);
 //   case ImGuiDataType_Double: return DragBehaviorT<double, double, double>(data_type, (double*)p_v, v_speed, p_min ? *(const double*)p_min : -DBL_MAX, p_max ? *(const double*)p_max : DBL_MAX, format, flags);
    case ImGuiDataType_Double: return DragBehaviorDouble(data_type, (double*)p_v, v_speed, p_min ? *(const double*)p_min : -DBL_MAX, p_max ? *(const double*)p_max : DBL_MAX, format, flags);
    case ImGuiDataType_COUNT:  break;
    }
    IM_ASSERT(0);
    return false;
...
float ScaleRatioFromValueS32(ImGuiDataType data_type, ImS32 v, ImS32 v_min, ImS32 v_max,
    bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    if (v_min == v_max)
        return 0.0f;

    const ImS32 v_clamped = (v_min < v_max) ?
        ImClampS32(v, v_min, v_max) :
        ImClampS32(v, v_max, v_min);

    if (is_logarithmic)
    {
        bool flipped = v_max < v_min;
        if (flipped)
            ImSwapS32(&v_min, &v_max);

        // Fudge min/max to avoid getting close to log(0)
        float v_min_fudged = (ImAbs((float)v_min) < logarithmic_zero_epsilon) ?
            ((v_min < 0.0f) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (float)v_min;
        float v_max_fudged = (ImAbs((float)v_max) < logarithmic_zero_epsilon) ?
            ((v_max < 0.0f) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (float)v_max;

        // 特殊ケースの処理
        if ((v_min == 0) && (v_max < 0))
            v_min_fudged = -logarithmic_zero_epsilon;
        else if ((v_max == 0) && (v_min < 0))
            v_max_fudged = -logarithmic_zero_epsilon;

        float result;
        float v_clamped_f = (float)v_clamped;

        if (v_clamped_f <= v_min_fudged)
            result = 0.0f;
        else if (v_clamped_f >= v_max_fudged)
            result = 1.0f;
        else if ((v_min * v_max) < 0) // レンジがゼロをまたぐ場合
        {
            ...
            if (v == 0)
                ...
                result = (1.0f - (float)(ImLog(-(float)v_clamped_f / logarithmic_zero_epsilon) /
                    ImLog(-v_min_fudged / logarithmic_zero_epsilon))) * zero_point_snap_L;
            else
                result = zero_point_snap_R + ((float)(ImLog((float)v_clamped_f / logarithmic_zero_epsilon) /
                    ImLog(v_max_fudged / logarithmic_zero_epsilon)) * (1.0f - zero_point_snap_R));
        }
        else if ((v_min < 0) || (v_max < 0)) // 完全に負の範囲
            result = 1.0f - (float)(ImLog(-(float)v_clamped_f / -v_max_fudged) /
                ImLog(-v_min_fudged / -v_max_fudged));
        else
            result = (float)(ImLog((float)v_clamped_f / v_min_fudged) /
    ...
    // 線形スライダー
    return (float)((ImS32)(v_clamped - v_min)) / (float)((ImS32)(v_max - v_min));
}
//
ImS32 ScaleValueFromRatioS32(ImGuiDataType data_type, float t,
    ImS32 v_min, ImS32 v_max,
    ...
    float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        else
        {
            ...
            float v_min_fudged = (ImAbs((float)v_min) < logarithmic_zero_epsilon) ?
                ((v_min < 0) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (float)v_min;
            float v_max_fudged = (ImAbs((float)v_max) < logarithmic_zero_epsilon) ?
                ((v_max < 0) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (float)v_max;

            if (flipped)
            {
                float temp = v_min_fudged;
            ...
            if ((v_min * v_max) < 0)
            {
                float zero_point_center = (-(float)ImMin(v_min, v_max)) /
                    ImAbs((float)v_max - (float)v_min);
                float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
                float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
                if (t_with_flip >= zero_point_snap_L && t_with_flip <= zero_point_snap_R)
                    result = 0;
                else if (t_with_flip < zero_point_center)
                    result = (ImS32)(-(logarithmic_zero_epsilon *
                        ImPow(-v_min_fudged / logarithmic_zero_epsilon,
                            (float)(1.0f - (t_with_flip / zero_point_snap_L)))));
                else
                    result = (ImS32)(logarithmic_zero_epsilon *
                        ImPow(v_max_fudged / logarithmic_zero_epsilon,
                            (float)((t_with_flip - zero_point_snap_R) / (1.0f - zero_point_snap_R))));
            }
            else if ((v_min < 0) || (v_max < 0))
                result = (ImS32)(-(-v_max_fudged *
                    ImPow(-v_min_fudged / -v_max_fudged, (float)(1.0f - t_with_flip))));
            else
                result = (ImS32)(v_min_fudged *
                    ImPow(v_max_fudged / v_min_fudged, (float)t_with_flip));
        }
    }
    else
    {
        ...
        if (t < 1.0f)
        {
            float v_new_off_f = (ImS32)(v_max - v_min) * t;
            result = (ImS32)(v_min + (ImS32)(v_new_off_f +
                (float)(v_min > v_max ? -0.5f : 0.5f)));
        }
        else
        {
            result = v_max;
...
int RoundScalarWithFormat(const char* format, int v)
{
    // フォーマットに基づいて丸める処理
    // ここでは整数型に特化した処理を行います
    // 例: フォーマットが "%d" の場合、そのまま返す
    // 例: フォーマットが "%.0f" の場合、四捨五入して返す

    // フォーマット文字列を解析
    const char* fmt_start = format;
    while (*fmt_start && *fmt_start != '%')
        fmt_start++;
    if (!*fmt_start)
        return v; // フォーマット文字列が無効な場合、そのまま返す

    // フォーマット指定子を解析
    char fmt_specifier = 0;
    while (*fmt_start && !fmt_specifier)
    {
        if (*fmt_start == 'd' || *fmt_start == 'i' || *fmt_start == 'u')
            fmt_specifier = *fmt_start;
        fmt_start++;
    }
    ...
    case 'u':
        ...
        return v;
    default:
        // その他のフォーマット指定子の場合、そのまま返す
        return v;
    }
}
bool SliderBehaviorS32(const ImRect& bb, ImGuiID id, ImGuiDataType data_type,
    ImS32* v, const ImS32 v_min, const ImS32 v_max,
    const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            const ImVec2 input_delta2 = ImGui::GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiInputReadMode_RepeatFast, 0.0f, 0.0f);
            float input_delta = (axis == ImGuiAxis_X) ? input_delta2.x : -input_delta2.y;
            if (input_delta != 0.0f)
            {
                const int decimal_precision = 0;
                if (decimal_precision > 0)
                {
                    input_delta /= 100.0f;
                ...
                {
                    if ((v_range >= -100 && v_range <= 100) || ImGui::IsNavInputDown(ImGuiNavInput_TweakSlow))
            ...
            {
                clicked_t = ScaleRatioFromValueS32(data_type, *v, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                ...
                else
                {
                    ...
                    ImS32 v_new = ScaleValueFromRatioS32(data_type, clicked_t, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_new = RoundScalarWithFormat(format, v_new);
                    float new_clicked_t = ScaleRatioFromValueS32(data_type, v_new, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
    ...
    {
        float grab_t = ScaleRatioFromValueS32(data_type, *v, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
        if (axis == ImGuiAxis_Y)
            grab_t = 1.0f - grab_t;
...
bool SliderBehaviorU32(const ImRect& bb, ImGuiID id, ImGuiDataType data_type,
    ImU32* v, const ImU32 v_min, const ImU32 v_max,
    const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                else
                {
                    ...
                    ImU32 v_new = ScaleValueFromRatioS32(data_type, clicked_t, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_new = RoundScalarWithFormat(format, v_new);
                    float new_clicked_t = ScaleRatioFromValueS32(data_type, v_new, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
...
bool SliderBehaviorFloat(const ImRect& bb, ImGuiID id, ImGuiDataType data_type,
    float* v, const float v_min, const float v_max,
    const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    const float slider_sz = (bb.Max[axis] - bb.Min[axis]) - grab_padding * 2.0f;
    float grab_sz = style.GrabMinSize;
    float v_range = (v_min < v_max ? v_max - v_min : v_min - v_max);
    if (!is_decimal && v_range >= 0)
        grab_sz = ImMax((float)(slider_sz / (v_range + 1)), style.GrabMinSize);
    grab_sz = ImMin(grab_sz, slider_sz);
    const float slider_usable_sz = slider_sz - grab_sz;
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            if (input_delta != 0.0f)
            {
                ...
                else
                {
                    if ((v_range >= -100.0f && v_range <= 100.0f) || ImGui::IsNavInputDown(ImGuiNavInput_TweakSlow))
                        input_delta = ((input_delta < 0.0f) ? -1.0f : +1.0f) / v_range;
                    else
                        input_delta /= 100.0f;
                }
                if (ImGui::IsNavInputDown(ImGuiNavInput_TweakFast))
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                else
                {
                    ...
                    float v_new = ScaleValueFromRatioS32(data_type, clicked_t, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_new = RoundScalarWithFormat(format, v_new);
                    float new_clicked_t = ScaleRatioFromValueS32(data_type, v_new, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
...
bool SliderBehaviorDouble(const ImRect& bb, ImGuiID id, ImGuiDataType data_type,
    double* v, const double v_min, const double v_max,
    const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            if (input_delta != 0.0f)
            {
                ...
                {
                    if ((v_range >= -100.0 && v_range <= 100.0) || ImGui::IsNavInputDown(ImGuiNavInput_TweakSlow))
                        input_delta = ((input_delta < 0.0f) ? -1.0f : +1.0f) / v_range;
                    else
                        input_delta /= 100.0f;
                }
            ...
            {
                clicked_t = ScaleRatioFromValueS32(data_type, (float)*v, (float)v_min, (float)v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                ...
                else
                {
                    ...
                    double v_new = ScaleValueFromRatioS32(data_type, clicked_t, (float)v_min, (float)v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_new = RoundScalarWithFormat(format, v_new);
                    float new_clicked_t = ScaleRatioFromValueS32(data_type, (float)v_new, (float)v_min, (float)v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
    ...
    {
        float grab_t = ScaleRatioFromValueS32(data_type, (float)*v, (float)v_min, (float)v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
        if (axis == ImGuiAxis_Y)
            grab_t = 1.0f - grab_t;
        const float grab_pos = ImLerp(slider_usable_pos_min, slider_usable_pos_max, grab_t);
        if (axis == ImGuiAxis_X)
...
//    case ImGuiDataType_S8: { ImS32 v32 = (ImS32) * (ImS8*)p_v;  bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min, *(const ImS8*)p_max, format, flags, out_grab_bb); if (r) *(ImS8*)p_v = (ImS8)v32;  return r; }
    ...
    {
        ImS32 v32 = (ImS32) * (ImS8*)p_v;
        bool r = SliderBehaviorS32(bb, id, ImGuiDataType_S32, &v32,
...
//    case ImGuiDataType_U8: { ImU32 v32 = (ImU32) * (ImU8*)p_v;  bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min, *(const ImU8*)p_max, format, flags, out_grab_bb); if (r) *(ImU8*)p_v = (ImU8)v32;  return r; }
    case ImGuiDataType_U8:
    {
        ...
        bool r = SliderBehaviorU32(bb, id, ImGuiDataType_U32, &v32,
...
//    case ImGuiDataType_U16: { ImU32 v32 = (ImU32) * (ImU16*)p_v; bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)p_min, *(const ImU16*)p_max, format, flags, out_grab_bb); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
    ...
    {
        IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorU32(bb, id, data_type, (ImU32*)p_v,
    ...
    {
        IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
        return SliderBehaviorFloat(bb, id, data_type, (float*)p_v,
    ...
    {
        IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
        return SliderBehaviorDouble(bb, id, data_type, (double*)p_v,
    ...
    if (g.ActiveId == id)
    {
        ...
        if (io.InputQueueCharacters.Size > 0)
        {
            if (!ignore_char_inputs && !is_readonly && !user_nav_input_start)
                for (n = 0; n < io.InputQueueCharacters.Size; n++)
                {
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (tab_src_n = 0; tab_src_n < tab_bar->Tabs.Size; tab_src_n++)
    {
    ...
    for (tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
    ...
    for (section_n = 0; section_n < 3; section_n++)
    {
        ...
        for (tab_n = 0; tab_n < section->TabCount; tab_n++)
        {
