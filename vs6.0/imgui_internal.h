...
#if 0
// VC++6.0用の実装
static void ImGui_DebugLog(const char* fmt, ...)
{
    char buffer[1024];
    va_list args;
    va_start(args, fmt);
    ...
    va_end(args);
    printf("[%05d] %s", GImGui->FrameCount, buffer);
}

#define IMGUI_DEBUG_LOG ImGui_DebugLog

#endif
#define IMGUI_DEBUG_LOG
#else
...
static inline ImVec2 ImMin( ImVec2 lhs, const ImVec2& rhs)                { return ImVec2(lhs.x < rhs.x ? lhs.x : rhs.x, lhs.y < rhs.y ? lhs.y : rhs.y); }
static inline ImVec2 ImMax( ImVec2 lhs, const ImVec2& rhs)                { return ImVec2(lhs.x >= rhs.x ? lhs.x : rhs.x, lhs.y >= rhs.y ? lhs.y : rhs.y); }
static inline ImVec2 ImClamp( ImVec2& v,  ImVec2& mn, ImVec2 mx)      { return ImVec2((v.x < mn.x) ? mn.x : (v.x > mx.x) ? mx.x : v.x, (v.y < mn.y) ? mn.y : (v.y > mx.y) ? mx.y : v.y); }
static inline ImVec2 ImLerp( ImVec2 a, const ImVec2& b, float t)          { return ImVec2(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t); }
static inline ImVec2 ImLerp(const ImVec2& a, const ImVec2& b, const ImVec2& t)  { return ImVec2(a.x + (b.x - a.x) * t.x, a.y + (b.y - a.y) * t.y); }
static inline ImVec4 ImLerp( ImVec4 a, const ImVec4& b, float t)          { return ImVec4(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, a.z + (b.z - a.z) * t, a.w + (b.w - a.w) * t); }
// ImVec2用のImLerp関数
static inline ImVec2 ImLerpVec2(ImVec2 a, ImVec2 b, float t)
{
    return ImVec2(a.x + (b.x - a.x) * t,
        a.y + (b.y - a.y) * t);
}

// ImVec2用のImLerp関数（ベクトルによる補間）
static inline ImVec2 ImLerpVec2Vec2(ImVec2 a, ImVec2 b, ImVec2 t)
{
    return ImVec2(a.x + (b.x - a.x) * t.x,
        a.y + (b.y - a.y) * t.y);
}
// ImVec4用のImLerp関数
static inline ImVec4 ImLerpVec4(ImVec4 a, ImVec4 b, float t)
{
...
#endif
// ImVec2用の専用ImClamp関数
static inline ImVec2 ImClampVec2(ImVec2 v, ImVec2 mn, ImVec2 mx)
{
    return ImVec2(
...
// imgui_internal.h に以下を追加

// ImS32型用のクランプ関数
inline ImS32 ImClampS32(ImS32 v, ImS32 mn, ImS32 mx)
{
    return (v < mn) ? mn : (v > mx) ? mx : v;
}

// ImS32型用のスワップ関数も必要な場合
inline void ImSwapS32(ImS32* a, ImS32* b)
{
    ImS32 tmp = *a;
    *a = *b;
    *b = tmp;
...
struct IMGUI_API ImRect
{
	...
	//	Min = ImClamp(Min, r.Min, r.Max); Max = ImClamp(Max, r.Min, r.Max); 
		Min = ImClampVec2(Min, r.Min, r.Max);
	    Max = ImClampVec2(Max, r.Min, r.Max);
