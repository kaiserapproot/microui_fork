...
typedef struct _OSVERSIONINFOEXW_CUSTOM {
...
typedef BOOL (WINAPI *PFN_VerifyVersionInfoW)(LPOSVERSIONINFOEXW, DWORD, ULONGLONG);
...
static PFN_SetLayeredWindowAttributes g_SetLayeredWindowAttributes = NULL;

// 関数の初期化
static bool InitVerSetConditionMask()
{
    if (g_VerSetConditionMask)
        return true;
    ...
    return g_VerSetConditionMask != NULL;
}
static bool InitVerifyVersionInfoW()
{
    if (g_VerifyVersionInfoW)
        return true;
    ...
    return g_VerifyVersionInfoW != NULL;
}
// 関数の初期化
static bool InitMonitorFromWindow()
{
...
// 関数の初期化
static bool InitSetLayeredWindowAttributes()
{
    ...
    g_SetLayeredWindowAttributes = (PFN_SetLayeredWindowAttributes)GetProcAddress(hUser32, "SetLayeredWindowAttributes");
...
//    OSVERSIONINFOEXW osvi = { sizeof(osvi), major, minor, 0, 0, { 0 }, sp, 0, 0, 0, 0 };
	// OSVERSIONINFOEXW構造体の初期化を分割
	// VC++6.0用の拡張OSVERSIONINFO構造体の定義
	#if _MSC_VER <= 1200
	typedef struct _OSVERSIONINFOEXW_CUSTOM {
		DWORD dwOSVersionInfoSize;
		DWORD dwMajorVersion;
	...
	// 構造体の初期化
	#if _MSC_VER <= 1200
	OSVERSIONINFOEXW_CUSTOM osvi;
	ZeroMemory(&osvi, sizeof(OSVERSIONINFOEXW_CUSTOM));
	osvi.dwOSVersionInfoSize = sizeof(OSVERSIONINFOEXW_CUSTOM);
	#else
	...
	// 使用箇所の修正
	ULONGLONG cond = 0;
	if (InitVerSetConditionMask() && g_VerSetConditionMask)
	{
		cond = g_VerSetConditionMask(0, VER_MAJORVERSION, VER_GREATER_EQUAL);
	...
	// 使用箇所の修正
	if (InitVerifyVersionInfoW() && g_VerifyVersionInfoW)
	{
		return g_VerifyVersionInfoW((OSVERSIONINFOEXW*)&osvi, mask, cond);
	}
	return FALSE;  // 関数が利用できない場合
#else
...
// 列挙型が未定義の場合のみ定義を追加
#ifndef MONITOR_DPI_TYPE_DECLARED
#define MONITOR_DPI_TYPE_DECLARED
...
#endif // MONITOR_DPI_TYPE_DECLARED
...
float ImGui_ImplWin32_GetDpiScaleForHwnd(void* hwnd)
{
	...
	if (InitMonitorFromWindow() && g_MonitorFromWindow)
	{
...
static void ImGui_ImplWin32_SetWindowAlpha(ImGuiViewport* viewport, float alpha)
{
    ...
    if (alpha < 1.0f)
    {
		...
		// 使用箇所の修正
		if (InitSetLayeredWindowAttributes() && g_SetLayeredWindowAttributes)
		{
